...
const showEffectedBoard = () =>{
    ...
    for (let i=0;i<effectedBoard.length;i++){

          let box = document.getElementById(effectedBoard[i])
          

            setTimeout(() => { box.style.backgroundColor = effectedBoxColor},500*(i+1))
            setTimeout(() => {box.style.backgroundColor = defaulBoxColor},600*(i+1))
...
const resetLevel = () =>{
      ...
      if(isRunning){
        //new
        squerNumber = defaultSequenceNumber
        levelLive = defaultLevelLive
        removeBoxes()
        resetBoards()
        initializeLevel()
